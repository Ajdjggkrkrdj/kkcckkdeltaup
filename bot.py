...
def download_filter(bot: DeltaBot, message: Message, replies: Replies) -> None:
    ...
    if message.chat.is_multiuser() or not message.text.startswith("http"):
        replies.add("Sorry no puedo hablar, solo mandame enlaces directo pndj 馃檪",quote=message)
        return
...
def queue_download(
    ...
    if addr in downloads:
        replies.add(text="Espere, ya tiene un proceso en curso 馃槾", quote=message)
    elif len(downloads) >= MAX_QUEUE_SIZE:
        replies.add(
            text="馃槵 Ya estoy procesando ** 5 subidas a la vez,** por favor reintente m谩s tarde 鈿狅笍",
            quote=message,
        )
    else:
        replies.add(text="**鉁� Solicitud agregada/procesando 鉁�**", quote=message)
    ...
    while True:
        ...
        for addr, parts in items:
            ...
            try:
                ...
                downloads.pop(addr, None)
                replies.add(text=f"# 鈿狅笍ERROR鈿狅笍\n```{ex}```", chat=chat)
                replies.send_reply_messages()
            except (StopIteration, Exception) as ex:
                bot.logger.exception(ex)
                downloads.pop(addr, None)
                replies.add(
                    text="*馃毇 Error en el enlace de descarga, revise que sea directo 馃槓*", chat=chat
                )
                replies.send_reply_messages()
